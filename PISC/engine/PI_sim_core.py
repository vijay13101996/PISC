...
class SimUniverse(object):
        def __init__(self,method,pathname,sysname,potkey,corrkey,enskey,Tkey,ext_kwlist=None,folder_name='Datafiles',sympletic_order=None):
                self.method = method
        ...
        def run_R2(self,sim,C='p',B='p',A='q',seed_number=None):
                ...
                assert self.dim ==1,'dimension = {} but R2 is only implemented for dim==1, sorry'.format(self.dim)
                ...
                def record_var():
                        ...
                        tarr.append(sim.t)
                        Marr.append(Mval)
                        qarr.append(q/sqrtnbeads)  #Needed to add further scaling to transform the 0 normal mode to centroid
        ...
        def run_seed(self,rngSeed,op=None):
                ...
                print('Start simulation.      Seed: {}  T {}, nbeads {}'.format(rngSeed,self.T,self.nbeads))
                ...
                if(self.corrkey =='OTOC'):
                        ...
                        tarr, Csym, Casym = self.run_R2(sim,self.extparam[0],self.extparam[1],self.extparam[2],seed_number=rngSeed)
