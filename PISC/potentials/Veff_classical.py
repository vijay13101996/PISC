...
class Veff_classical_1D_LH(PES):
	...
	def potential(self,q):
		...
		if (hess >0):
			wa = np.sqrt(hess)
			xi = 0.5*self.beta*wa
			Vnc = m*ka**2*(np.tanh(xi)/xi -1)/(8*hess) - 0.5*np.log(np.tanh(xi)/xi)/self.beta + np.log(np.sinh(xi)/xi)/self.beta		
			return Vc+Vnc	
		elif (hess <0): 	
			wa = np.sqrt(-hess)
			xi = 0.5*self.beta*wa
			# The potential is defined in terms of tan instead of tanh when the hessian is negative.
			Vnc = m*ka**2*(np.tan(xi)/xi-1)/(8*hess) - 0.5*np.log(np.tan(xi)/xi)/self.beta + np.log(np.sin(xi)/xi)/self.beta
			return Vc + Vnc
		elif (hess==0):
