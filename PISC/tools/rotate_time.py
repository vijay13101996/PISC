...
def rotate_time_inv(file_name, ndimensions, dt, output_name):
    """Interface to specific functions"""
    if ndimensions == 2:
        rotate_time_R2_inv(file_name, dt, output_name)
    else:
        raise NotImplementedError(
            "rotate time for {} dimensions is not implemented".format(ndimensions)
        )
...
def rotate_time_R2_inv(file_name, dt, output_name):
    """Rotate time from (t1,t2) to (t1,-t1+t2)"""
    data = np.loadtxt(file_name)
    assert data.shape[1] == 4, "Data must have four columns"
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "-inv", "--inverse", action="store_true", help="Do 'inverse rotation' "
    )

    args = args.parse_args()
    print("\nRotating frame ...\n")
    if args.inverse:
        rotate_time_inv(args.file_name, args.ndimension, args.dt, args.output_name)
    else:
