...
def main(filename,pathname,sysname,nrun,lamda,g,times,m,N,nbeads,dt,rngSeed,time_therm,time_relax,qgrid,nsample):
	dim = 1
	...
	for k in range(len(qgrid)):
		...
		sim.step(ndt=nsteps_therm,mode="nvt",var='pq',RSP=True,pc=False,centmove=False)
		print('k, q[k]',k, rp.q[0,0,0])
		for i in range(nsample):
			sim.step(ndt=nsteps_relax,mode="nvt",var='pq',RSP=True,pc=False,centmove=False)
