...
def main():
    ...
    fig, ax = plt.subplots(figsize=(3.5,3.5))#2, figsize=(3.5, 3.5))
    ...
    if(1):
        ...
        if(1):
            try:
                ...
                ax.scatter(coeffarr, bnarr3[1:], label=r'$L={},J_2=0 (Truncated)$'.format(L), s=10, color='orange')
        ...
        ax.legend(fontsize=le_fs-4,loc='upper right')
    ...
    if(0):
        ...
        try:
            axin = ax.inset_axes([0.5, 0.05, 0.43, 0.43])
            ...
            axin.plot(wgrid, tauO, '-',label=r'$L={},J_2={}$'.format(L,J2), color='blue')
            #Fit wgrid vs tauO1 between w0 =4 and 6
            init = 21
            fit = np.polyfit(wgrid[init:45], np.log(tauO[init:45]), 1)
            print('fit = ', fit, len(wgrid))
            linear_func = np.poly1d(fit)
            axin.plot(wgrid[init:], np.exp(linear_func(wgrid[init:])), '--', color='blue',lw=2)
            ...
            axin.plot(wgrid[init:], np.exp(linear_func(wgrid[init:])), '--', color='green',lw=2)
            axin.plot(wgrid, tauO1, '-',label=r'$L={},J_2={}$'.format(L,J2), color='green')

            fit2 = np.polyfit(wgrid[init:30], np.log(tauO2[init:30]), 1)
            print('fit2 = ', fit2)
            linear_func2 = np.poly1d(fit2)
            axin.plot(wgrid[init:], np.exp(linear_func2(wgrid[init:])), '--', color='orange',lw=2)
            axin.plot(wgrid[:32], tauO2[:32], '-',label=r'$L={},J_2={}$ (Truncated)'.format(L,J2), color='orange')
            
            axin.set_yscale('log')
            #axin.axhline(y=np.pi/2, color='r', linestyle='--', label=r'$\pi/2$')
            axin.set_xlabel(r'$\omega$', fontsize=xl_fs)
            axin.set_ylabel(r'$f_{{O}}(0,\omega)$', fontsize=yl_fs)
            axin.legend(fontsize=le_fs-2,loc='upper right')
            axin.set_ylim(axin.get_ylim()[0], axin.get_ylim()[1]*1e10)
            axin.annotate(r'$(b)$', xy=(0.05, 0.9), xycoords='axes fraction', fontsize=tp_fs+2)
    ...
    fig.savefig('draft_Krylov_Bop_XXZ_NNN_2.pdf', dpi=400, bbox_inches='tight',pad_inches=0.0)
