...
basis = spin_basis_1d(L,pauli=False,Nup=L//2,kblock=k)#,pblock=1) # and positive parity sector
...
if(1):# NN XXZ model
    try:
        ...
        vals_NN = pickle.load(open('eigenvalues_NN_L_{}_zfull.pkl'.format(L), 'rb'))
        vecs_NN = pickle.load(open('eigenvectors_NN_L_{}_zfull.pkl'.format(L), 'rb'))
        H_mat_NN = pickle.load(open('H_mat_NN_L_{}_zfull.pkl'.format(L), 'rb'))
        ...
        pickle.dump(vals_NN, open('eigenvalues_NN_L_{}_zfull.pkl'.format(L), 'wb'))
        pickle.dump(vecs_NN, open('eigenvectors_NN_L_{}_zfull.pkl'.format(L), 'wb'))
        pickle.dump(H_mat_NN, open('H_mat_NN_L_{}_zfull.pkl'.format(L), 'wb'))

    O_eigenbasis= construct_Jz(vecs_NN, vals_NN, H_mat_NN, basis, L, J, Delta, J2, Delta2)
    ...
    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_Jz_NN_beta_{}.pkl'.format(beta), 'wb'))
...
if(1):# NNN XXZ model
    ...
    O_eigenbasis= construct_Jz(vecs_NNN, vals_NNN, H_mat_NNN, basis, L, J, Delta, J2, Delta2)

    b_arr = np.zeros(ncoeff)
    b_arr = K_complexity(O_eigenbasis,vals_NNN,beta=beta,ncoeff=ncoeff)
    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_Jz_NNN_beta_{}.pkl'.format(beta), 'wb'))
