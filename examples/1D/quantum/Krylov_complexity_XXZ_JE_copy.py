...
if(0):# NN XXZ model
    ...
    O_eigenbasis= construct_JE(vecs_NN, vals_NN, H_mat_NN, basis, L, J, Delta, J2, Delta2)
    ...
    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_JE_NN_beta_{}.pkl'.format(beta), 'wb'))
...
if(1):# NNN XXZ model
    ...
    O= construct_JE(vecs_NNN, vals_NNN, H_mat_NNN, basis, L, J, Delta, J2, Delta2)
    ...
    for nmat in [4,5,6,7,8]:
        ...
        plt.colorbar()
        plt.title('JE operator in NNN XXZ, n={}'.format(nmat))
        plt.xlabel('i')
        plt.ylabel('j')
        plt.savefig('JE_On_matrix_n_{}.png'.format(nmat))
        plt.clf()
