...
for pot in ['box','lorenz']:
    ...
    tarr_short = np.linspace(-20,20,2000)
    ...
    if(1):
        ...
        plt.plot(coeff_arr, np.pi*T*coeff_arr, ls='--')
        plt.xlabel('n', fontsize=xl_fs)
        plt.ylabel('Lanczos b_n', fontsize=yl_fs)
        plt.legend(fontsize=le_fs)
    if(0):   
        plt.plot(tarr_short, C_tcf_short, label='TCF C(t) short')
        #plt.plot(tarr, C_tcf, label='TCF C(t) long', alpha=0.5)
