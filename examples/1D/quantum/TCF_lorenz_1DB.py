...
def potential_lorenz(x,y):
    if( x**2 + y**2 < R**2):
        return 1e6
    elif(x>-a and x<a and y>-a and y<a):
        return 0.0 
    else:
        return 1e6
potential_lorenz = np.vectorize(potential_lorenz)  

def potential_box(x,y):
    if (x>a or x<-a or y>a or y<-a):
        return 1e6
    else:
        return 0.0
...
def bookkeeping(pot):
    if pot=='box':
        ...
        potkey = 'MANU_square_box_a_{}_R_{}'.format(np.around(a,2),np.around(R,2))
    elif pot=='lorenz':
        potential_xy = potential_lorenz
        potkey = 'MANU_lorenz_billiards_a_{}_R_{}'.format(np.around(a,2),np.around(R,2))
...
for pot in ['box','lorenz']:
    ...
    try:
        ...
        if(0): #Plot Lanczos coeffs
            plt.plot(data_lanczos[:,0], data_lanczos[:,1], label='Lanczos coeffs {}'.format(pot))
            plt.scatter(data_lanczos[:,0], data_lanczos[:,1], s=10)
            Evals = vals[-1]/2
            plt.axhline(y=Evals, color='r', linestyle='--', label='E_avg/2')
            plt.xlabel('n', fontsize=xl_fs)
            plt.ylabel('Lanczos b_n', fontsize=yl_fs)
            plt.legend(fontsize=le_fs)
   
        if(1): #Plot TCF FT
            plt.plot(data[:,0], np.exp(data[:,1]), label='TCF FT {}'.format(pot))
            plt.xlabel('Frequency ω', fontsize=xl_fs)
            plt.ylabel('Log TCF FT log|C(ω)|', fontsize=yl_fs)
            #for d in np.diff(np.sort(vals))[:200]:
            #    plt.axvline(x=d, color='r', linestyle='--', alpha=0.5)
            plt.legend(fontsize=le_fs)
    
        if(0): #Plot TCF
            plt.plot(data_tcf[:,0], ((data_tcf[:,1])), label=' C(t) {}'.format(pot))
        ...
        if(0):
            tarr = np.linspace(0,200,1000)
            C_arr = np.zeros_like(tarr) + 0j   
            basis_N = 50
            n_eigen = 30

            k_arr = np.arange(basis_N) +1
            m_arr = np.arange(basis_N) +1
            C_arr = OTOC_f_1D_omp_updated.otoc_tools.lambda_corr_arr_t(vecs,m,x_arr,DVR.dx,DVR.dx,k_arr,vals,m_arr,tarr,beta,n_eigen,'xxC',0.5,C_arr)
            plt.plot(tarr, C_arr.real, label='Re C(t) Lanczos recomputed {}'.format(pot)) 
            plt.show()
            verify_Ct(O, vals, barr, beta, neigs, tarr=tarr)
plt.show()
