...
if(0):
    ...
    for i in range(10):
        LnO = Krylov_complexity.krylov_complexity.compute_hadamard_product(L,LnmO,LnO)
        print('LnmO',np.real(LnO/LnmO)[:5,:5])
        plt.imshow(np.log(abs(LnO-LnmO)))
        plt.show()
        LnmO = LnO.copy()
...
def lanczos_coeffs(O, L, vals, beta, lamda, ip, ncoeff):
    ...
    for T_au in T_arr:
        ...
        if(1):
            for nmat in range(0,21,2):#range(100):
                ...
                #print('n',bval**0.5/(np.pi*T_au))
                logOn = np.abs(np.log(On))
                #print('logOn',logOn[0:5,0:5])

                #ax[0].imshow(np.abs(np.log(On2)))#vmax=1e4)
                ...
                ax[1].scatter(np.arange(0,neigs), np.diag(On2),s=3)
                #ax[1].plot(np.diag(np.matmul(On.T,On)/Z),label='nmat={}'.format(nmat))
                #ax[1].hlines((np.trace(On2))**0.5,xmin=0,xmax=neigs)
                ax[1].set_title(r'$O_{:d}$'.format(nmat))
    ...
    for i in range(len(T_arr)):
        plt.scatter(np.arange(1,ncoeff+1),bnarr[i,:],label='T={}'.format(T_arr[i]),s=10)
        #plt.plot(np.arange(1,ncoeff+1), x*np.pi*T_arr[i]*np.arange(0,ncoeff))
    #plt.hlines(vals[k_diag-1]//2,xmin=0,xmax=ncoeff)

#lanczos_coeffs(O, L, vals, 1.0/T_arr[0], 0.5/x, 'dir', ncoeff)
#lanczos_coeffs(O, L, vals, 1.0/T_arr[0], 0.5/x, 'asm', ncoeff)
lanczos_coeffs(O, L, vals, 1.0/T_arr[0], 0.5, 'asm', ncoeff)
#lanczos_coeffs(o, l, vals, 1.0/t_arr[0], 0.5,'asm', ncoeff)
