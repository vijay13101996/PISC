...
m = 1
w = 0.3j #quadratic term (frequency)
a = 0.0 #cubic term
b = 0.25 #quartic term
...
betalist = [0.1,0.5,1.0,3.0,5.0,10.0,20.0,40.0]

slist = [10,10,10,10,2,2,1,0.9]
...
for beta,ax,s in zip(betalist,axlist,slist):
    ...
    ax.annotate(r'$\beta={}$'.format(beta),xy=(0.7,0.7),xytext=(0.5,0.8),xycoords='axes fraction',fontsize=13)

ax4.set_xticks([-3,0,3])
ax8.set_xticks([-3,0,3])

for ax in [ax1,ax2,ax3,ax4]:
    ax.set_yticks([1,5,9])

fig.suptitle(r'$Quartic \: + \: inverted \; harmonic$',fontsize=15)
...
fig.savefig('Quarticharm_allbeta_eps_{}.png'.format(abs(w)**2),dpi=400,bbox_inches='tight',pad_inches=0.0)
plt.show()
