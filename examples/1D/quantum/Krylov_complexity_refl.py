...
for n,L in zip([0.5,1,1.5,2,3,4],[200,100,70,30,10,10]):
    lb=-L
    ub=L
    ...
    potkey = 'Refl_edge_m_{}_n_{}'.format(m,n)
    ...
    lenarr = np.arange(1,len(diff)+1)

    plt.plot(lenarr,diff,'o',label='n={}'.format(n))
...
T_arr = [0.2]#,2,4,10,20,40,100]#np.arange(1.,30.05,2.)#[0.5,1.0,1.5,2.0,2.5,3.0,3.5,4.0]
mun_arr = []
mu0_harm_arr = []
bnarr = []
nmoments = 60
...
if(1):
    ...
    for i in [0]:#range(0,len(T_arr),2):
        ...
        # Find slope
        slope = np.polyfit((np.arange(2,20)),(bnarr[i,2:20]),1)
        slope_arr.append(slope[0])
    
        print('slope',slope, np.pi*T_arr[i])
        plt.plot(np.arange(2,30),slope[0]*np.arange(2,30)+slope[1],lw=2.5)
