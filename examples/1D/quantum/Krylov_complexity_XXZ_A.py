...
if(1):# NN XXZ model
    ...
    O_eigenbasis= construct_A(vecs_NN, vals_NN, H_mat_NN, basis, L)
    b_arr = np.zeros(ncoeff)
    b_arr = K_complexity(O_eigenbasis,vals_NN,beta=beta,ncoeff=ncoeff)
    plt.scatter(coeff_arr, b_arr, label=r'$NN \; XXZ$')

    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_A_NN_beta_{}.pkl'.format(beta), 'wb'))
...
if(1):# NNN XXZ model
    ...
    O_eigenbasis= construct_A(vecs_NNN, vals_NNN, H_mat_NNN, basis, L)
    
    b_arr = np.zeros(ncoeff)
    b_arr = K_complexity(O_eigenbasis,vals_NNN,beta=beta,ncoeff=ncoeff)
    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_A_NNN_beta_{}.pkl'.format(beta), 'wb'))
    
    plt.annotate(r'$\hat{O}=\hat{B}$', xy=(0.5, 0.5), xytext=(0.6, 0.6), textcoords='axes fraction')
    plt.scatter(coeff_arr, b_arr, label=r'$NNN \; XXZ$')
    plt.xlabel(r'$n$')
    plt.ylabel(r'$b_n$')
    plt.legend()
    #plt.show()
