...
if(1):# NNN XXZ model
    ...
    if(1):
        ...
        plt.colorbar()
        plt.title('Operator in energy eigenbasis (NNN XXZ)')
        plt.xlabel('i')
        plt.ylabel('j')
        #plt.show()
        plt.savefig('JE_eigenbasis_NNN.png')
        exit()

    b_arr = np.zeros(ncoeff)
    b_arr = K_complexity(O_eigenbasis,vals_NNN,beta=beta,ncoeff=ncoeff)
    if store:
        pickle.dump((coeff_arr, b_arr), open('krylov_coeffs_JE_NNN_beta_{}.pkl'.format(beta), 'wb'))
