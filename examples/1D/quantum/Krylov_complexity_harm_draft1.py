...
# Position operator analytical
pos_mat_anal = np.zeros((neigs,neigs))
for i in range(neigs):
    for j in range(i,neigs):
        if abs(i-j) == 1:
            pos_mat_anal[i,j] = np.sqrt(j/2.0)
        else:
            pos_mat_anal[i,j] = 0.0
        pos_mat_anal[j,i] = pos_mat_anal[i,j]
...
# O2^k operator
O2k = O2.copy()
x2k = pos_mat_anal.copy()
k=100
for n in range(k-1):
    O2k = np.matmul(O2k,O2)
    x2k = np.matmul(x2k,pos_mat_anal)
...
if(1): # Lanczos coefficients for O2^k - exact and approx
    ...
    def plot_ax_2a(ax):
        for T_au in [5.0]:#,2.0]:#,4.0]:
            coeffarr,bnarr = compute_bn(O2k,vals,vecs,T_au,50)
            coeffarr2,bnarr2 = compute_bn(O2K_approx,vals,vecs,T_au,50)
            coeffarrx, bnarrx = compute_bn(x2k,vals,vecs,T_au,50)

            #ax.scatter(coeffarr[1:],bnarr[1:],label=r'$T={}$'.format(T_au), s=scat_size)
            ax.scatter(coeffarrx[1:],bnarrx[1:],  ls='--',s=scat_size)
            ax.plot(coeffarr2[1:],bnarr2[1:],  ls='--')

        #### Save data in txt file
        np.savetxt('FIG_2a_exact_x2k_bn_T{}.txt'.format(T_au), np.c_[coeffarr,bnarrx], header='n   bn')
        np.savetxt('FIG_2a_approx_x2k_bn_T{}.txt'.format(T_au), np.c_[coeffarr2,bnarr2], header='n   bn')

        ax.plot(coeffarr[:4], np.pi*coeffarr[:4]*T_au, 'r--', label=r'$\pi n T$')
        ax.set_xlabel(r'$n$', fontsize=xl_fs)
        ax.set_ylabel(r'$b_n$', fontsize=yl_fs)
        #ax.set_title(r'$V(x) = \frac{1}{2}x^2, \;  \hat{O} = (\hat{x})^{100}$', fontsize=tp_fs)
        #ax.legend(fontsize=le_fs)
        #ax.annotate(r'$V(x) = \frac{1}{2}x^2, \;  \hat{O} = \hat{x}^{100}$', xy=(0.5, 0.9), xycoords='axes fraction', fontsize=ti_fs, ha='center')

        
#    plot_ax_2a(ax)
    #plt.savefig('FIG_2a_D1.pdf', dpi=300, bbox_inches='tight')
#    plt.show()
