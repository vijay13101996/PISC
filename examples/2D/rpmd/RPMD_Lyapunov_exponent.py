...
#instanton = find_instanton_DW(nbeads,m,pes,beta,ax=None,plt=None,plot=False) 
instanton=0.001*np.zeros((1,dim,nbeads))#just an example for now
instanton[...,0]=0.001

#initialization of instanton 
q=instanton
p=np.zeros_like(q)#or initialize differently
q[...,0]=-0.001

#initialize small deviation
dq=np.random.randn(*np.shape(q))#todo:vary
#dq[...,0]=0
dp=np.random.randn(*np.shape(q))#todo:vary
#dp[...,0]=0
dp,dq=norm_dp_dq(dp,dq,norm)
print(dp,dq)

#random initialization of p and q
#q=0.003*np.ones((8,dim,nbeads))#todo:vary
#p=0.001*np.ones((8,dim,nbeads))#todo:vary
