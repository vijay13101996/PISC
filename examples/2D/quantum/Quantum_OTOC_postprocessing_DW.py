...
if(1):
    for z,c in zip([0.0,2.0],['k','r','g','b','m']):
        print('z',z) 
        potkey = 'double_well_2D_alpha_{}_D_{}_lamda_{}_g_{}_z_{}'.format(alpha,D,lamda,g,z)
        ext = 'Quantum_Kubo_OTOC_{}_beta_{}_neigen_{}_basis_{}'.format(potkey,beta,40,70)
        ext = path+'/Datafiles/'+ext
        slope, ic, t_trunc, OTOC_trunc = find_OTOC_slope(ext,0.9,1.5)
        plot_1D(ax,ext,label='z={}, $\lambda={}$'.format(z,np.around(slope,2)),color=c, log=True,linewidth=1)
        #ax.plot(t_trunc, slope*t_trunc+ic,linewidth=1.5,color='k')
    plt.legend()
    plt.title('Quantum OTOCs for different $z$ at $T=3T_c$')
    plt.show()
