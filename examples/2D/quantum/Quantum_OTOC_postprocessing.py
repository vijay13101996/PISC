...
if(1):
        ...
        quit()
        print(eigrid.shape)
        plt.contour(x, y, np.array(eigrid[:, :, 0]), color = 'y', levels = [-0.05])#np.arange(-0.51,-0.49, 0.01)) 
        hesgrid = 0.25 * (omega**2 + 4 * g0 * omega * (x**2 + y**2) - 48 *g0**2 * x**2 * y**2)
        plt.contour(x, y, potgrid, colors = 'k',levels = [5])#np.arange(4.9,5.1,0.02))#,levels=vals[:20])#np.arange(0,5,0.5))
        #plt.contour(x,y,hesgrid,colors='m',levels=np.arange(-0.5,0.0,0.1))
