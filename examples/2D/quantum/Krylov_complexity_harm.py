...
from PISC.potentials import harmonic2D
...
potkey = 'TESTharmonic_w_{}_m_{}'.format(omega,m)
pes = harmonic2D(omega,m=m)

if(0): #Plot the potential
    xg = np.linspace(lbx,ubx,ngridx)
    yg = np.linspace(lby,uby,ngridy)
    xgr,ygr = np.meshgrid(xg,yg)
    plt.contour(pes.potential_xy(xgr,ygr),levels=np.arange(0,30,1.0))
    plt.show()    
    exit()
...
for T_au in T_arr:    
    ...
    print('moments',even_moments[0],even_moments[1])
    ...
    #print('mu0_harm',mu0_harm_arr[-1], mu0_harm_arr[-1]**2)
