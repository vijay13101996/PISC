...
if(1):
	...
	for times,c,rang in zip(times_arr,color_arr,ext_arr):
		...
		ax.errorbar(times,rpslope,yerr=error,fmt='o',ms=3,color=rpc,capsize=2.0,label='RPMD')
		#ms 4 for thesis
...
if(1):
	...
	ax.annotate('$\lambda_{bound}=2\pi \: k_B T/\hbar$',xy=(0.27,0.75), xycoords='axes fraction', xytext=(0.31,0.82), textcoords= 'axes fraction', fontsize=10)
		
	T_ext = np.arange(1.05,3.06,0.2)
	ax.scatter(T_ext, 2.0*np.ones_like(T_ext),marker='x',color='sienna',s=ms,zorder=4)
	...
	T_arr = np.linspace(0.7,1.51,1000) #
	ax.plot(T_arr, 2*np.pi*T_arr*Tc,color='k',ls='-')

	ax.set_xlabel(r'$T/T_c$',fontsize=xl_fs)
	ax.set_ylabel(r'$\lambda(T)$',fontsize=xl_fs)

	xticks = np.arange(0.75,3.01,0.25) #
	yticks = np.arange(0.0,3.01)
