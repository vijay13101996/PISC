...
ax[0].set_title(r'$T=3T_c$', fontsize=ti_fs, x=0.22,y=0.85)
...
ax[1].set_title(r'$T=0.95T_c$',fontsize=ti_fs, x=0.3,y=0.85)
...
ext = 'Quantum_Kubo_OTOC_{}_beta_{}_neigen_{}_basis_{}'.format(potkey,beta,20,50)
ext =qext+ext
plot_1D(ax[2],ext,label='Quantum',color=qc, log=True,linewidth=lwd)

#RPMD
ext = 'RPMD_thermal_OTOC_{}_{}_nbeads_{}_dt_{}'.format(potkey,Tkey,32,0.002)
ext =rpext+ext
plot_1D(ax[2],ext,label='RPMD',color=rpc, log=True,linewidth=lwd)

#Classical
ext = 'Classical_thermal_OTOC_{}_{}_dt_{}'.format(potkey,Tkey,0.002)
ext =Cext+ext
plot_1D(ax[2],ext,label='Classical',color=Cc, log=True,linewidth=lwd)

ax[2].set_xlabel(r'$t$',fontsize=xl_fs)
ax[2].set_xticks(xticks)
ax[2].set_ylim([-3.4,1.5])
#ax[2].set_yticks(yticks,yticksf)
ax[2].set_title(r'$T=0.6T_c$', fontsize=ti_fs,x=0.27,y=0.85)
...
#handles, labels = ax.get_legend_handles_labels()
#fig.legend(loc = (0.25, 0.9),ncol=3, fontsize=le_fs)
